...
[현재 구현 메모]
- 실제 구현 파일은 `app/main.py`, `app/api/endpoints.py`이다.
- API 서버 실행은 `uvicorn app.main:app --reload --port 8000`이며, `python app/main.py`는 콘솔 데모를 실행한다.
- 현재 SSE 페이로드는 예시의 `{"node": ...}` 고정 스키마가 아니라 `{node_name: update_dict}` 구조를 사용한다.
...
data: {"orchestrator": {"status": "searching_graph", "message": "지식 그래프 검색 중..."}} \n\n
data: {"finance_analyst": {"status": "analyzing", "content": "부채비율 200% 확인..."}} \n\n
...
data: {"system": {"status": "error", "message": "[Fallback] AI 분석 지연으로 기본 리스크 시나리오를 반환합니다."}} \n\n
...
## 4.4 [RFC] SSE 이벤트 스키마 v2 (Sprint 4 도입 예정, 하위호환 유지)

[CURSOR AI 작업 지침]
이 절은 `14_Upgrade_Master_Plan.md` Pillar 6(UX 2.0)의 사전 합의 RFC다.
**현재(v1) 스키마는 그대로 유지**되며, v2는 새 이벤트 타입을 추가하는 방식으로 동시 송출된다.
프론트엔드는 `type` 필드 존재 여부로 v1/v2를 구분한다 (없으면 v1).

### v1 스키마 (현재 유지)

기존: `data: {<node_name>: <update_dict>}\n\n`
모든 기존 클라이언트는 변경 없이 동작한다.

### v2 스키마 (확장, type 기반)

```jsonc
{ "type": "node_start",         "node": "finance_analyst", "ts": 1730000000 }
{ "type": "token",              "node": "finance_analyst", "delta": "..." }   // 토큰 스트리밍
{ "type": "evidence_added",     "evidence_id": "qd:abc123", "preview": "..." }
{ "type": "node_end",           "node": "finance_analyst", "elapsed_ms": 4321, "cost_usd": 0.012 }
{ "type": "interrupt_requested","reason": "low_confidence", "thread_id": "..." }
{ "type": "ping",               "ts": 1730000010 }                            // 15초 keep-alive
{ "type": "done",               "thread_id": "..." }
{ "type": "error",              "code": "LLM_TIMEOUT", "fallback": true, "message": "..." }
```

### 호환성 원칙
1. v1 페이로드는 변경 금지(node_name 키 그대로 유지).
2. 같은 SSE 스트림에서 v1/v2를 **동시 송출**한다.
3. 프론트엔드 v1 구독자는 v2 이벤트를 무시하면 되고, v2 구독자는 v1을 보조 시그널로 사용한다.
4. v2 단독 송출로 전환되는 시점은 별도 PR에서 합의 후 결정한다 (목표: Sprint 6 종료).

### 신규 엔드포인트(예정)
- `POST /api/v1/analyze/interrupt/{thread_id}` : 사용자 개입 요청
- `POST /api/v1/analyze/resume/{thread_id}` : 편집된 state로 재개

엔드포인트는 Sprint 4(Memory)에서 LangGraph PostgresSaver와 함께 도입된다.